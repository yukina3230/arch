...
local osc_styles = {
    ...
    Time = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&H000000&\\fs17\\fs'.. user_opts.infofontsize ..'\\q2\\fn' .. user_opts.font .. '}',
    Tooltip = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H000000&\\fs18\\fs'.. user_opts.infofontsize ..'\\q2\\fn' .. user_opts.font .. '}',
    Title = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs'.. user_opts.titlefontsize ..'\\q2\\fn' .. user_opts.font .. '}',
    WindowTitle = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs'.. user_opts.titlefontsize ..'\\q2\\fn' .. user_opts.font .. '}',
...
function osc_init()
    ...
    ne.eventresponder['mbtn_left_up'] =
        function () 
            ...
            msg.error(mp.get_property_number('volume', nil));
            if (mp.get_property_number('volume', nil) == 0) then
                mp.commandv("osd-auto", "add", "volume", 50)
            end
        end
    ...
    ne.slider.posF =
        function ()
            ...
            if (state.mute) then
                val = 0 -- set slider to zero (only visually) when muted, slider will move back when unmuted
            end
            if (val == 0 and not state.mute) then
                mp.commandv('cycle', 'mute')
            end
            return val*val/100
        end
    ne.eventresponder['mouse_move'] =
        function (element)
            ...
            local seekto = get_slider_value(element)
            if (state.mute) then
                mp.commandv('cycle', 'mute') -- unmute when dragged
                state.mute = false
            end
            if (element.state.lastseek == nil) or
                (not (element.state.lastseek == seekto)) then
...
function adjustSubtitles()
    if user_opts.raisesubswithosc and state.osc_visible == true and (state.fullscreen == false or user_opts.showfullscreen) then
        local w, h = mp.get_osd_size()
        if h > 0 then
            mp.commandv('set', 0, math.floor((osc_param.playresy - 175)/osc_param.playresy*100)) -- percentage
        end
	else
		mp.commandv('set', 0, 100)
	end	
end
