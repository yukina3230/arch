...
local osc_styles = {
    ...
    VolumeText = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&H000000&\\fs17\\fs'.. user_opts.infofontsize + 2 ..'\\q2\\fn' .. user_opts.font .. '}',
...
function downloadDone()
    show_message("\\N{\\an9}Download saved to " .. mp.command_native({"expand-path", "~/Videos/mpv/"}))
...
layouts = function ()
    ...
    -- Volume Text
    lo = add_layout('vol_text')
    lo.geometry = {x = 250, y = refY - 40, an = 4, w = 20, h = 20}
    lo.style = osc_styles.VolumeText
    lo.visible = (osc_param.playresx >= 900 - outeroffset) and user_opts.volumecontrol
...
function osc_init()
    ...
    ne.eventresponder['mbtn_left_up'] =
        function ()
            if (mp.get_property_number('volume') == 0 or state.mute) then
                mp.set_property_native('mute', false)
                mp.commandv('set', 'volume', vol_value)
            else
    ...
    -- volumetext
    ne = new_element('vol_text', 'button')
    ne.visible = user_opts.showtitle
    ne.content = function ()
